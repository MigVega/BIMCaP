...
def visualize_aligned_walls(wall_data, save_path, scale=1):
    ...
    plt.scatter(pcd[:, 0], pcd[:, 1], c=points_colors) #TODO 2
    plt.savefig(f"{save_path}/alligned_walls.png")


def visualize_data(data, save_path, suffix="", scale=1):
    left, right, wall, floor, ceiling = data
    left_pcd = unproject_points_rotvec(left["depths"], left["points"], left["f"],
                                       left["pp"], left["rotvecs"], left["translations"], scale=scale)

    right_pcd = unproject_points_rotvec(right["depths"], right["points"], right["f"],
                                        right["pp"], right["rotvecs"], right["translations"], scale=scale)

    floor_pcd = unproject_points_rotvec(floor["depths"], floor["points"], floor["f"],floor["pp"], floor["rotvecs"], floor["translations"], scale=scale)

    wall_pcd = unproject_points_rotvec(wall["depths"], wall["points"], wall["f"],wall["pp"], wall["rotvecs"], wall["translations"], scale=scale)

    ceiling_pcd = unproject_points_rotvec(ceiling["depths"], ceiling["points"], ceiling["f"],ceiling["pp"], ceiling["rotvecs"], ceiling["translations"], scale=scale)


    if "segments" in wall:
        visualize_aligned_walls(wall, save_path, scale=scale)
    ...
    ceiling_pcd = make_pcd_from_numpy(ceiling_pcd, blue)
    line_set = make_line_set(left_pcd, right_pcd, green)


    o3d.io.write_line_set(f"{save_path}/keypoints"+suffix+".ply", line_set)
    o3d.io.write_point_cloud(f"{save_path}/wall"+suffix+".ply", wall_pcd)
    o3d.io.write_point_cloud(f"{save_path}/floor"+suffix+".ply", floor_pcd)
    o3d.io.write_point_cloud(f"{save_path}/ceiling"+suffix+".ply", ceiling_pcd)
